...
class HeroDayStat {
  const HeroDayStat({required this.icon, required this.value, required this.label});
  final IconData icon;
  final String value;
  final String label;
}
      ...
      child: Column(
        children: [
          for (var row = 0; row < (stats.length / 2).ceil(); row++)
            Padding(
              padding: EdgeInsets.only(bottom: row == (stats.length / 2).ceil() - 1 ? 0 : 12),
              child: Row(
                children: [
                  Expanded(child: _StatTile(stat: stats[row * 2], dark: dark)),
                  const SizedBox(width: 12),
                  if (row * 2 + 1 < stats.length)
                    Expanded(child: _StatTile(stat: stats[row * 2 + 1], dark: dark))
                  else
...
class _StatTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final c = VikaColors.of(context);
    final iconBg = dark
        ? c.invInk.withValues(alpha: 0.08)
        : c.bg;
...
class _CtaButton extends StatelessWidget {
  const _CtaButton({required this.label, required this.dark, required this.onTap});
          ...
          child: Row(
            ...
            children: [
              ...
              Container(
                ...
                alignment: Alignment.center,
                child: Icon(Icons.arrow_forward_rounded, size: 16, color: knobFg),
              ),
            ],
          ),
        ),

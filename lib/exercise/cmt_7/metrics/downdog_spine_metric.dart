...
class DowndogSpineMetric extends Cmt7MetricBase {
  @override
  ...
  void update(Cmt7Context ctx) {
    final isActive = ctx.state == Cmt7State.p5_parvatasana ||
        ctx.state == Cmt7State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt7Config.V_SHAPE_ARM_ANGLE_RANGE[0];

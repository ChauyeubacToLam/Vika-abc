...
class LumbarBreakMetric extends Cmt7MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt7State from, Cmt7State to, int timestampMs) {
    if (to == Cmt7State.p2_hasta_uttanasana ||
        to == Cmt7State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt7Context ctx) {
    final isActive = ctx.state == Cmt7State.p2_hasta_uttanasana ||
        ctx.state == Cmt7State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt7Config.HIP_FORWARD_PUSH_THRESHOLD;

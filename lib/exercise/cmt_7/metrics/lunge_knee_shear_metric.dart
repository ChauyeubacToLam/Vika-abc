...
class LungeKneeShearMetric extends Cmt7MetricBase {
  @override
  ...
  void update(Cmt7Context ctx) {
    final isActive = ctx.state == Cmt7State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt7State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt7Config.KNEE_OVER_FOOT_TOLERANCE;
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt7Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt7Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

...
class HastaUttanasana extends ExerciseBase {
    ...
    if (wrist.y < nose.y) {
      ...
      if (armAngle >= 150 && armAngle <= 180) {
        ...
        resultIssues.addInstruction(currentPhaseKey, 'Pose',
            "Duỗi thẳng cánh tay (góc hiện tại: ${armAngle.toStringAsFixed(0)}°).");
        _resetState();
        return;
      }
    } else {
      resultIssues.addInstruction(
          currentPhaseKey, 'Pose', "Nâng hai tay cao qua đầu.");
      _resetState();
      return;
    }
    ...
    if (isVerified) {
      if (state == HastaUttanasanaState.setup) {
        ...
        if (_currentHoldTime >= targetHoldTime) {
          ...
          resultIssues.addInstruction(
              currentPhaseKey, 'Status', "Hoàn thành Hasta Uttanasana");
        }
      }
    }
  ...
  String get currentPhaseLabel {
    switch (state) {
      case HastaUttanasanaState.setup:
        return 'Chuẩn bị';
      case HastaUttanasanaState.holding:
        return 'Giữ tĩnh';
    }
  }

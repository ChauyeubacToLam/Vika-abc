...
class DowndogSpineMetric extends Cmt5MetricBase {
  @override
  ...
  void update(Cmt5Context ctx) {
    final isActive = ctx.state == Cmt5State.p5_parvatasana ||
        ctx.state == Cmt5State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt5Config.V_SHAPE_ARM_ANGLE_RANGE[0];

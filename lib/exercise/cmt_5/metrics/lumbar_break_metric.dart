...
class LumbarBreakMetric extends Cmt5MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt5State from, Cmt5State to, int timestampMs) {
    if (to == Cmt5State.p2_hasta_uttanasana ||
        to == Cmt5State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt5Context ctx) {
    final isActive = ctx.state == Cmt5State.p2_hasta_uttanasana ||
        ctx.state == Cmt5State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt5Config.HIP_FORWARD_PUSH_THRESHOLD;

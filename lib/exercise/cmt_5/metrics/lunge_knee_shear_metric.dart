...
class LungeKneeShearMetric extends Cmt5MetricBase {
  @override
  ...
  @override
  void update(Cmt5Context ctx) {
    final isActive = ctx.state == Cmt5State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt5State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt5Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt5State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt5Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt5Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

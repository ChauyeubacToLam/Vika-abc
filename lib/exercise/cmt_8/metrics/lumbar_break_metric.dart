...
class LumbarBreakMetric extends Cmt8MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt8State from, Cmt8State to, int timestampMs) {
    if (to == Cmt8State.p2_hasta_uttanasana ||
        to == Cmt8State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt8Context ctx) {
    final isActive = ctx.state == Cmt8State.p2_hasta_uttanasana ||
        ctx.state == Cmt8State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt8Config.HIP_FORWARD_PUSH_THRESHOLD;

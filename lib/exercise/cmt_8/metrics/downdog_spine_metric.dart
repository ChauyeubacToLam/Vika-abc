...
class DowndogSpineMetric extends Cmt8MetricBase {
  @override
  ...
  void update(Cmt8Context ctx) {
    final isActive = ctx.state == Cmt8State.p5_parvatasana ||
        ctx.state == Cmt8State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt8Config.V_SHAPE_ARM_ANGLE_RANGE[0];

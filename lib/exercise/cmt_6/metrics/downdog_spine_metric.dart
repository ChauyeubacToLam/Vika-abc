...
class DowndogSpineMetric extends Cmt6MetricBase {
  @override
  ...
  void update(Cmt6Context ctx) {
    final isActive = ctx.state == Cmt6State.p5_parvatasana ||
        ctx.state == Cmt6State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt6Config.V_SHAPE_ARM_ANGLE_RANGE[0];

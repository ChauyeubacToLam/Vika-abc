...
class LumbarBreakMetric extends Cmt6MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt6State from, Cmt6State to, int timestampMs) {
    if (to == Cmt6State.p2_hasta_uttanasana ||
        to == Cmt6State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt6Context ctx) {
    final isActive = ctx.state == Cmt6State.p2_hasta_uttanasana ||
        ctx.state == Cmt6State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt6Config.HIP_FORWARD_PUSH_THRESHOLD;

...
class LungeKneeShearMetric extends Cmt6MetricBase {
  @override
  ...
  @override
  void update(Cmt6Context ctx) {
    final isActive = ctx.state == Cmt6State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt6State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt6Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt6State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt6Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt6Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

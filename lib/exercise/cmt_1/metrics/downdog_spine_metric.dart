...
class DowndogSpineMetric extends Cmt1MetricBase {
  @override
  ...
  void update(Cmt1Context ctx) {
    final isActive = ctx.state == Cmt1State.p5_parvatasana ||
        ctx.state == Cmt1State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt1Config.V_SHAPE_ARM_ANGLE_RANGE[0];

...
class LumbarBreakMetric extends Cmt1MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt1State from, Cmt1State to, int timestampMs) {
    if (to == Cmt1State.p2_hasta_uttanasana ||
        to == Cmt1State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt1Context ctx) {
    final isActive = ctx.state == Cmt1State.p2_hasta_uttanasana ||
        ctx.state == Cmt1State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt1Config.HIP_FORWARD_PUSH_THRESHOLD;

...
class LungeKneeShearMetric extends Cmt1MetricBase {
  @override
  ...
  void update(Cmt1Context ctx) {
    final isActive = ctx.state == Cmt1State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt1State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt1Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt1State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt1Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt1Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

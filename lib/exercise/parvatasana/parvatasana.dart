  ...
  void checkingPose(Map<PoseLandmarkType, PoseLandmark> smoothedLandmarks) {
    ...
    if (deltaXAnkle > bodyHeight * 0.5) {
      resultIssues.addInstruction(
          currentPhaseKey, 'Pose', "Rút chân về sát nhau.");
      _resetState();
      return;
    }
    ...
    if (!isHipHighest) {
      resultIssues.addInstruction(
          currentPhaseKey, 'Pose', "Đẩy hông lên cao để tạo hình chữ V ngược.");
      _resetState();
      return;
    }
    ...
    if (armAngle < 160) {
      resultIssues.addInstruction(
          currentPhaseKey, 'Pose', "Duỗi thẳng hai cánh tay.");
      _resetState();
      return;
    }
    ...
    final backAngle = calculateAngleNormalized(
    ...
    if (backAngle >= 155 && backAngle <= 180) {
      ...
      if (backAngle < 155) {
        resultIssues.addInstruction(currentPhaseKey, 'Pose',
            "Đẩy mạnh hai tay xuống thảm, ép vai ra sau, vươn dài lưng lên đỉnh mông!");
        _resetState();
        return;
      } else {
    ...
    if (isVerified) {
      if (state == ParvatasanaState.setup) {
        ...
        resultIssues.addInstruction(
            currentPhaseKey, 'Status', "Lưng thẳng, Vai mở. Giữ tĩnh...");
        ...
        if (_currentHoldTime >= targetHoldTime) {
          ...
          state = ParvatasanaState.setup;
          resultIssues.addInstruction(
              currentPhaseKey, 'Status', "Hoàn thành Parvatasana");
        }
      }
    }

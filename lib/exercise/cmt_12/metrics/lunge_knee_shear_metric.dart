import 'cmt_12_metric_base.dart';
...
class LungeKneeShearMetric extends Cmt12MetricBase {
  @override
  ...
  @override
  void update(Cmt12Context ctx) {
    final isActive = ctx.state == Cmt12State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt12State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt12Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt12State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt12Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt12Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

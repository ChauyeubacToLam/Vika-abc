...
class DowndogSpineMetric extends Cmt12MetricBase {
  @override
  ...
  void update(Cmt12Context ctx) {
    final isActive = ctx.state == Cmt12State.p5_parvatasana ||
        ctx.state == Cmt12State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt12Config.V_SHAPE_ARM_ANGLE_RANGE[0];

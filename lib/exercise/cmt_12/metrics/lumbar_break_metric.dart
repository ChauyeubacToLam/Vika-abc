...
class LumbarBreakMetric extends Cmt12MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt12State from, Cmt12State to, int timestampMs) {
    if (to == Cmt12State.p2_hasta_uttanasana ||
        to == Cmt12State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt12Context ctx) {
    final isActive = ctx.state == Cmt12State.p2_hasta_uttanasana ||
        ctx.state == Cmt12State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt12Config.HIP_FORWARD_PUSH_THRESHOLD;

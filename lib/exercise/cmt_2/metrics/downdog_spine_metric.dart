...
class DowndogSpineMetric extends Cmt2MetricBase {
  @override
  ...
  void update(Cmt2Context ctx) {
    final isActive = ctx.state == Cmt2State.p5_parvatasana ||
        ctx.state == Cmt2State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt2Config.V_SHAPE_ARM_ANGLE_RANGE[0];

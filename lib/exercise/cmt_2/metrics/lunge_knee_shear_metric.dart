...
class LungeKneeShearMetric extends Cmt2MetricBase {
  @override
  ...
  void update(Cmt2Context ctx) {
    final isActive = ctx.state == Cmt2State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt2State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt2Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt2State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt2Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt2Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

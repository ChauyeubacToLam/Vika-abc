...
import 'metrics/cmt_2_metric_base.dart';
...
/// Cmt2 â€” Chuá»—i ChÃ o Máº·t Trá»i 12 bÆ°á»›c.
///
/// Camera: Side (sagittal plane, 100%).
/// 1 Rep = 12 poses liÃªn tiáº¿p (P1 â†’ P12 â†’ count).
class Cmt2 extends ExerciseBase with SideTrackedExerciseMixin {
  // â”€â”€ Orientation â”€â”€
  @override
  ...
  Cmt2State _state = Cmt2State.p1_pranamasana;
  Cmt2State _prevState = Cmt2State.p1_pranamasana;
  ...
  late final List<Cmt2MetricBase> _metrics = [
    lumbarMetric,
    kneeBendMetric,
    lungeMetric,
    downdogMetric,
  ...
  String get currentPhaseLabel {
    switch (_state) {
      case Cmt2State.p1_pranamasana:
        return 'P1: Cáº§u nguyá»‡n';
      case Cmt2State.p2_hasta_uttanasana:
        return 'P2: VÆ°Æ¡n tay';
      case Cmt2State.p3_hastapaadasana:
        return 'P3: Gáº­p ngÆ°á»i';
      case Cmt2State.p4_ashwa_sanchalanasana:
        return 'P4: Ká»µ sÄ©';
      case Cmt2State.p5_parvatasana:
        return 'P5: ChÃ³ cÃºi máº·t';
      case Cmt2State.p6_ashtanga_namaskara:
        return 'P6: 8 Ä‘iá»ƒm';
      case Cmt2State.p7_bhujangasana:
        return 'P7: Ráº¯n há»• mang';
      case Cmt2State.p8_parvatasana_return:
        return 'P8: ChÃ³ cÃºi máº·t';
      case Cmt2State.p9_ashwa_return:
        return 'P9: Ká»µ sÄ©';
      case Cmt2State.p10_hastapaadasana_return:
        return 'P10: Gáº­p ngÆ°á»i';
      case Cmt2State.p11_hasta_uttanasana_return:
        return 'P11: VÆ°Æ¡n tay';
      case Cmt2State.p12_pranamasana_return:
  ...
  bool isInStartPosition(Map<PoseLandmarkType, PoseLandmark> landmarks) {
    ...
    debugData['Setup_Diagnostic'] = {
      ...
      'isStanding': bodyAngle >= Cmt2Config.STANDING_STRAIGHT_RANGE[0] &&
          bodyAngle <= Cmt2Config.STANDING_STRAIGHT_RANGE[1],
    };

    return bodyAngle >= Cmt2Config.STANDING_STRAIGHT_RANGE[0] &&
        bodyAngle <= Cmt2Config.STANDING_STRAIGHT_RANGE[1];
  }
  ...
  bool requestStop() {
    if (_exerciseStartTimeMs != null &&
        (frameTimestampMs - _exerciseStartTimeMs!) >
            Cmt2Config.MAX_DURATION_MS) {
      _isTimeout = true;
      return true;
    }
    return repCount >= Cmt2Config.MAX_REP;
  }
  ...
  void _updateStateMachine(Cmt2Context ctx) {
    switch (_state) {
      // â”€â”€ P1: Cáº§u nguyá»‡n â†’ P2: VÆ°Æ¡n tay â”€â”€
      case Cmt2State.p1_pranamasana:
        // Trigger: Cá»• tay vÆ°Æ¡n lÃªn cao vÆ°á»£t qua MÅ©i (Nose)
        if (ctx.wristY < ctx.noseY - Cmt2Config.WRIST_ABOVE_NOSE_MARGIN) {
          _transitionTo(Cmt2State.p2_hasta_uttanasana, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p2_hasta_uttanasana:
        ...
        if (ctx.hipFlexionAngle < Cmt2Config.FOLD_HIP_ANGLE_THRESHOLD &&
            ctx.wristY > ctx.shoulderY) {
          _transitionTo(Cmt2State.p3_hastapaadasana, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p3_hastapaadasana:
        ...
        if (ctx.shoulderY < ctx.hipY && ctx.hipFlexionAngle > 80) {
          _transitionTo(
              Cmt2State.p4_ashwa_sanchalanasana, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p4_ashwa_sanchalanasana:
        ...
        if (ctx.hipY < ctx.shoulderY && ctx.hipY < ctx.ankleY) {
          _transitionTo(Cmt2State.p5_parvatasana, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p5_parvatasana:
        ...
        if (ctx.shoulderY > ctx.hipY && ctx.kneeY > ctx.hipY) {
          _transitionTo(Cmt2State.p6_ashtanga_namaskara, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p6_ashtanga_namaskara:
        ...
        if (ctx.hipY >= ctx.shoulderY && ctx.shoulderY < ctx.kneeY) {
          _transitionTo(Cmt2State.p7_bhujangasana, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p7_bhujangasana:
        ...
        if (ctx.hipY < ctx.shoulderY && ctx.hipY < ctx.ankleY) {
          _transitionTo(Cmt2State.p8_parvatasana_return, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p8_parvatasana_return:
        ...
        if (ctx.shoulderY < ctx.hipY && ctx.hipFlexionAngle > 80) {
          _transitionTo(Cmt2State.p9_ashwa_return, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p9_ashwa_return:
        ...
        if (ctx.hipFlexionAngle < Cmt2Config.FOLD_HIP_ANGLE_THRESHOLD &&
            ctx.wristY > ctx.hipY) {
          _transitionTo(
              Cmt2State.p10_hastapaadasana_return, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p10_hastapaadasana_return:
        ...
        if (ctx.wristY < ctx.noseY - Cmt2Config.WRIST_ABOVE_NOSE_MARGIN &&
            ctx.bodyAngle > 140) {
          _transitionTo(
              Cmt2State.p11_hasta_uttanasana_return, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p11_hasta_uttanasana_return:
        ...
        if (ctx.wristY > ctx.shoulderY &&
            ctx.bodyAngle >= Cmt2Config.STANDING_STRAIGHT_RANGE[0]) {
          _transitionTo(
              Cmt2State.p12_pranamasana_return, ctx.frameTimestamp);
        }
        break;
      ...
      case Cmt2State.p12_pranamasana_return:
        ...
        if (ctx.bodyAngle >= Cmt2Config.STANDING_STRAIGHT_RANGE[0]) {
          _completeRep(ctx);
        }
        break;
    }
  ...
  void _transitionTo(Cmt2State newState, int timestampMs) {
  ...
  void _completeRep(Cmt2Context ctx) {
    repCount++;
    ...
    _transitionTo(Cmt2State.p1_pranamasana, ctx.frameTimestamp);
    for (final metric in _metrics) {

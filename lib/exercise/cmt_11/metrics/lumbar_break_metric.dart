...
class LumbarBreakMetric extends Cmt11MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt11State from, Cmt11State to, int timestampMs) {
    if (to == Cmt11State.p2_hasta_uttanasana ||
        to == Cmt11State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt11Context ctx) {
    final isActive = ctx.state == Cmt11State.p2_hasta_uttanasana ||
        ctx.state == Cmt11State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt11Config.HIP_FORWARD_PUSH_THRESHOLD;

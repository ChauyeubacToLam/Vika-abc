...
class LungeKneeShearMetric extends Cmt11MetricBase {
  @override
  ...
  void update(Cmt11Context ctx) {
    final isActive = ctx.state == Cmt11State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt11State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt11Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt11State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt11Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt11Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

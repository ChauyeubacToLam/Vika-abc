...
class LumbarBreakMetric extends Cmt4MetricBase {
  @override
  ...
  void onStateTransition(Cmt4State from, Cmt4State to, int timestampMs) {
    if (to == Cmt4State.p2_hasta_uttanasana ||
        to == Cmt4State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt4Context ctx) {
    final isActive = ctx.state == Cmt4State.p2_hasta_uttanasana ||
        ctx.state == Cmt4State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt4Config.HIP_FORWARD_PUSH_THRESHOLD;

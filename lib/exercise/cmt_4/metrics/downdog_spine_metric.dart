...
class DowndogSpineMetric extends Cmt4MetricBase {
  @override
  ...
  void update(Cmt4Context ctx) {
    final isActive = ctx.state == Cmt4State.p5_parvatasana ||
        ctx.state == Cmt4State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt4Config.V_SHAPE_ARM_ANGLE_RANGE[0];

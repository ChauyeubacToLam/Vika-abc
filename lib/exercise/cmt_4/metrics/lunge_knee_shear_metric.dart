...
class LungeKneeShearMetric extends Cmt4MetricBase {
  @override
  ...
  void update(Cmt4Context ctx) {
    final isActive = ctx.state == Cmt4State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt4State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt4Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt4State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt4Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt4Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

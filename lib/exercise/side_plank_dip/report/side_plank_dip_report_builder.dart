...
class SidePlankDipReportBuilder {
  ...
  SidePlankSetReport buildReport(int startTimeMs, int endTimeMs) {
    ...
    if (shoulderFaults > 0) {
      prescriptions.add(
          "🔴 KHỚP VAI (CRITICAL): Khuỷu tay của bạn bị lệch trục trong $shoulderFaults nhịp. Điều này tạo lực cắt cực mạnh lên chóp xoay. LUÔN LUÔN căn chỉnh khuỷu tay vuông góc chính xác dưới vai trước khi bắt đầu.");
    }

    if (rotationFaults > 0) {
      prescriptions.add(
          "🔴 CỘT SỐNG (CRITICAL): Bạn bị mất kiểm soát, đổ người về trước/sau. Khi tập, hãy tưởng tượng lưng bạn đang tựa phẳng vào một bức tường.");
    }

    if (depthScore < 60) {
      prescriptions.add(
          "🟡 BIÊN ĐỘ: Nhịp Dip (hạ hông) của bạn chỉ mang tính nhấp nháy. Để cơ liên sườn phát triển, hãy hạ hông xuống sát mặt sàn rồi dùng lực mông đẩy mạnh lên.");
    }

    if (prescriptions.isEmpty) {
      prescriptions.add(
          "✅ HOÀN HẢO: Bạn kiểm soát tư thế Side Plank xuất sắc! Khớp vai và cột sống được bảo vệ tuyệt đối.");
    }

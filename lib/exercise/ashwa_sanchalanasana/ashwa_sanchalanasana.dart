...
class AshwaSanchalanasana extends ExerciseBase {
  ...
  void checkingPose(Map<PoseLandmarkType, PoseLandmark> smoothedLandmarks) {
    ...
    final ear = isLeft ? smoothedLandmarks[PoseLandmarkType.leftEar]! : smoothedLandmarks[PoseLandmarkType.rightEar]!;
    
    bool isFacingRight = nose.x > ear.x;
    
    // Front leg is the one further forward in the facing direction
    bool leftIsFront = isFacingRight ? leftAnkle.x > rightAnkle.x : leftAnkle.x < rightAnkle.x;
    
    final frontHip = leftIsFront ? smoothedLandmarks[PoseLandmarkType.leftHip]! : smoothedLandmarks[PoseLandmarkType.rightHip]!;
    final backHip = leftIsFront ? smoothedLandmarks[PoseLandmarkType.rightHip]! : smoothedLandmarks[PoseLandmarkType.leftHip]!;
    
    PoseLandmark frontKneeRaw = leftIsFront ? smoothedLandmarks[PoseLandmarkType.leftKnee]! : smoothedLandmarks[PoseLandmarkType.rightKnee]!;
    final backKnee = leftIsFront ? smoothedLandmarks[PoseLandmarkType.rightKnee]! : smoothedLandmarks[PoseLandmarkType.leftKnee]!;
    
    final frontAnkle = leftIsFront ? smoothedLandmarks[PoseLandmarkType.leftAnkle]! : smoothedLandmarks[PoseLandmarkType.rightAnkle]!;
    final backAnkle = leftIsFront ? smoothedLandmarks[PoseLandmarkType.rightAnkle]! : smoothedLandmarks[PoseLandmarkType.leftAnkle]!;
    
    final frontFootIndex = leftIsFront ? smoothedLandmarks[PoseLandmarkType.leftFootIndex]! : smoothedLandmarks[PoseLandmarkType.rightFootIndex]!;
    ...
    if (_previousFrontKnee != null && _previousHip != null && _previousAnkle != null) {
      ...
      double kneeMovement = calculateDistance(frontKneeRaw, _previousFrontKnee!);
      ...
      if (hipMovement < 0.1 * bodyScale && ankleMovement < 0.1 * bodyScale && kneeMovement > 0.15 * bodyScale) {
    ...
    // Front knee angle
    double frontKneeAngle = calculateAngle(firstPoint: frontHip, midPoint: frontKnee, lastPoint: frontAnkle);
    bool isFrontKneeValid = frontKneeAngle >= 90 && frontKneeAngle <= 110;
    
    // Back knee angle
    double backKneeAngle = calculateAngle(firstPoint: backHip, midPoint: backKnee, lastPoint: backAnkle);
    ...
    if (isKneeOverToe) {
       resultIssues.addInstruction(currentPhaseKey, 'Pose', "Bước chân sau dài ra, giữ đầu gối trước vuông góc với mắt cá!");
       _resetState();
       return; // Hard Block
    }
    ...
    if (isHipLowered && isFrontKneeValid && isBackKneeValid && isChestLifted) {
        ...
        if (!isChestLifted) {
             resultIssues.addInstruction(currentPhaseKey, 'Pose', "Vươn ngực lên, để vai cao hơn hông.");
        } else if (!isHipLowered) {
             resultIssues.addInstruction(currentPhaseKey, 'Pose', "Hạ thấp hông xuống một chút nữa.");
        } else if (!isFrontKneeValid) {
             resultIssues.addInstruction(currentPhaseKey, 'Pose', "Điều chỉnh đầu gối trước vuông góc (khoảng 90 độ).");
        } else if (!isBackKneeValid) {
             resultIssues.addInstruction(currentPhaseKey, 'Pose', "Duỗi thẳng chân sau ra sau hơn.");
        }
        _resetState();
        return;
    }
    ...
    if (isVerified) {
      if (state == AshwaSanchalanasanaState.setup) {
        ...
        if (_currentHoldTime >= targetHoldTime) {
          ...
          resultIssues.addInstruction(currentPhaseKey, 'Status', "Hoàn thành Ashwa Sanchalanasana");
        }
      }
    }
  ...
  String get currentPhaseLabel {
    switch (state) {
      case AshwaSanchalanasanaState.setup: return 'Chuẩn bị';
      case AshwaSanchalanasanaState.holding: return 'Giữ tĩnh';
    }
  }

...
class LumbarBreakMetric extends Cmt10MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt10State from, Cmt10State to, int timestampMs) {
    if (to == Cmt10State.p2_hasta_uttanasana ||
        to == Cmt10State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt10Context ctx) {
    final isActive = ctx.state == Cmt10State.p2_hasta_uttanasana ||
        ctx.state == Cmt10State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt10Config.HIP_FORWARD_PUSH_THRESHOLD;

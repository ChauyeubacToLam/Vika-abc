...
class LungeKneeShearMetric extends Cmt10MetricBase {
  @override
  ...
  void update(Cmt10Context ctx) {
    final isActive = ctx.state == Cmt10State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt10State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt10Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt10State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt10Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt10Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

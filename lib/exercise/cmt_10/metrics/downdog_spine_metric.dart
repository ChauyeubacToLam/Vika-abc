...
class DowndogSpineMetric extends Cmt10MetricBase {
  @override
  ...
  void update(Cmt10Context ctx) {
    final isActive = ctx.state == Cmt10State.p5_parvatasana ||
        ctx.state == Cmt10State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt10Config.V_SHAPE_ARM_ANGLE_RANGE[0];

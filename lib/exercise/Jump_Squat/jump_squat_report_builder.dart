...
class JumpSquatSetReport {
  ...
  double get landingSafetyScore => totalReps == 0 ? 0 : (safeLandingCount / totalReps) * 100;
  
  // Sức mạnh bùng nổ (%) - Thẻ số 2
  double get takeOffPowerScore => totalReps == 0 ? 0 : (perfectTakeOffCount / totalReps) * 100;
  
  // Kiểm soát trọng tâm (%) - Thẻ số 3
  double get torsoStabilityScore => totalReps == 0 ? 0 : (stableTorsoCount / totalReps) * 100;
}
  ...
  JumpSquatSetReport buildReport() {
    ...
    if (totalReps == 0) {
      return JumpSquatSetReport(
        totalReps: 0, safeLandingCount: 0, perfectTakeOffCount: 0, stableTorsoCount: 0, prescriptions: []
      );
    }
    ...
    for (var faults in _repHistory) {
      bool hasLandingFault = faults.any((f) => f.type == 'Knee' && f.affectsForm == true); // Tiếp đất chân cứng
    ...
    if (stiffLandingFaults > 0) {
      prescriptions.add("🔴 NGUY HIỂM: Bạn có $stiffLandingFaults lần tiếp đất thẳng chân! Lực đang dội thẳng vào khớp gối. Điều chỉnh: Nghĩ về việc 'ngồi xuống' ngay khoảnh khắc mũi chân chạm sàn để giảm xóc.");
    }
    
    // 🟠 HIGH: Rạp lưng khi tiếp đất (Nguy cơ đau thắt lưng)
    if (torsoLeanFaults > 0) {
      prescriptions.add("🟠 CHÚ Ý: Bạn bị rạp úp lưng quá mức trong $torsoLeanFaults nhịp. Điều chỉnh: Mở ngực, nhìn thẳng về trước khi tiếp đất để dồn tải vào đùi thay vì cột sống.");
    }

    // 🟡 MEDIUM: Lấy đà quá nông (Giảm hiệu suất bùng nổ)
    if (shallowTakeOffFaults > (totalReps * 0.3)) { // Báo lỗi nếu tần suất sai > 30%
      prescriptions.add("🟡 HIỆU SUẤT: Nhịp lấy đà chưa đủ độ nén. Điều chỉnh: Hạ hông sâu hơn (đùi gần song song mặt sàn) trước khi bật nhảy để kích hoạt tối đa sợi cơ mông.");
    }

    // Nếu không có lỗi nào
    if (prescriptions.isEmpty) {
      prescriptions.add("✅ XUẤT SẮC: Kỹ thuật Plyometric của bạn rất hoàn hảo! Khớp gối và cột sống được kiểm soát cực kỳ an toàn.");
    }

...
class CoreDriveMetric extends StandingKteMetricBase {
  static const double SHOULDER_DROP_RATIO_LIMIT = 0.3; // Shoulder dropped >30% of torso length
  ...
  void update(StandingKteRepContext ctx) {
    if (ctx.state == KteState.standing_base) {
      ...
      bool kneeBelowHip = ctx.liftingKnee.y > ctx.liftingHip.y; // Knee is physically lower than hip
      
      if (_setupShoulderY != null && ctx.torsoLength > 0) {
        double shoulderDrop = ctx.liftingShoulder.y - _setupShoulderY!; // positive if dropped down
        ...
        if (kneeBelowHip && dropRatio > SHOULDER_DROP_RATIO_LIMIT) {
          addFault(
            FaultRecord(
              type: 'neck_pulling',
              message: 'Hãy nhấc cao đùi lên! Đừng bẻ gập cổ và lưng chúi xuống!',

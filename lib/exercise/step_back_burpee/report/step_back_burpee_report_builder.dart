...
class StepBackBurpeeReportBuilder {
  ...
  BurpeeSetReport buildReport(int startTimeMs, int endTimeMs) {
    ...
    if (totalReps == 0) {
      return BurpeeSetReport(totalReps: 0, spineSafetyScore: 0, plankExtensionScore: 0, repsPerMinute: 0, prescriptions: []);
    }

    int spineFaultReps = 0; // Đếm số Rep vi phạm 1 trong 2 lỗi cột sống (Cúi cong lưng hoặc Võng Plank)
    ...
    double plankExtensionScore = ((totalReps - shortStepFaultReps) / totalReps) * 100;
    ...
    if (spineFaultReps > 0) {
      prescriptions.add("🔴 AN TOÀN CỘT SỐNG: Bạn có $spineFaultReps nhịp làm cột sống chịu áp lực xấu. Điều chỉnh: Khi hạ người, hãy chùng gối như đang ngồi xổm (Squat). Khi Plank, gồng cứng cơ bụng để Hông không rớt.");
    }

    if (plankExtensionScore < 70) {
      prescriptions.add("🟡 BIÊN ĐỘ: Động tác của bạn hơi 'tù'. Hãy dứt khoát bước dài chân ra sau để kéo giãn toàn bộ cơ thể thay vì co cụm.");
    }

    if (rpm < 10) { // Dưới 10 reps/phút là rất chậm với cardio
      prescriptions.add("🔵 NHỊP ĐỘ: Tốc độ hiện tại của bạn là ${rpm.toStringAsFixed(1)} reps/phút. Tập trung giữ form, khi thể lực tốt hơn, hãy cố gắng tăng tốc để đốt mỡ hiệu quả hơn.");
    } else {
      prescriptions.add("🔥 NHỊP ĐỘ: Tuyệt vời! Bạn đang duy trì tốc độ Cardio rất tốt.");
    }

    if (spineSafetyScore == 100 && plankExtensionScore == 100) {
      prescriptions.insert(0, "✅ XUẤT SẮC: Form tập của bạn chuẩn mực! Vừa an toàn cho xương khớp vừa đạt hiệu suất tối đa.");
    }

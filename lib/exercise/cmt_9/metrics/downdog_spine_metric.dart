...
class DowndogSpineMetric extends Cmt9MetricBase {
  @override
  ...
  void update(Cmt9Context ctx) {
    final isActive = ctx.state == Cmt9State.p5_parvatasana ||
        ctx.state == Cmt9State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt9Config.V_SHAPE_ARM_ANGLE_RANGE[0];

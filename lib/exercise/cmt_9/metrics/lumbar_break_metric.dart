...
class LumbarBreakMetric extends Cmt9MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt9State from, Cmt9State to, int timestampMs) {
    if (to == Cmt9State.p2_hasta_uttanasana ||
        to == Cmt9State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt9Context ctx) {
    final isActive = ctx.state == Cmt9State.p2_hasta_uttanasana ||
        ctx.state == Cmt9State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt9Config.HIP_FORWARD_PUSH_THRESHOLD;

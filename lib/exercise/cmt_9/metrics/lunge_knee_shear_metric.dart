...
class LungeKneeShearMetric extends Cmt9MetricBase {
  @override
  ...
  @override
  void update(Cmt9Context ctx) {
    final isActive = ctx.state == Cmt9State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt9State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt9Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt9State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt9Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt9Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

...
class LumbarBreakMetric extends Cmt3MetricBase {
  @override
  ...
  @override
  void onStateTransition(Cmt3State from, Cmt3State to, int timestampMs) {
    if (to == Cmt3State.p2_hasta_uttanasana ||
        to == Cmt3State.p11_hasta_uttanasana_return) {
  ...
  void update(Cmt3Context ctx) {
    final isActive = ctx.state == Cmt3State.p2_hasta_uttanasana ||
        ctx.state == Cmt3State.p11_hasta_uttanasana_return;
    if (!isActive) return;
    ...
    final isLumbarBreak = shoulderBehindHip &&
        hipForwardDelta < Cmt3Config.HIP_FORWARD_PUSH_THRESHOLD;

...
class LungeKneeShearMetric extends Cmt3MetricBase {
  @override
  ...
  void update(Cmt3Context ctx) {
    final isActive = ctx.state == Cmt3State.p4_ashwa_sanchalanasana ||
        ctx.state == Cmt3State.p9_ashwa_return;
    if (!isActive) return;
    ...
    final isShearing = kneeOverFoot > Cmt3Config.KNEE_OVER_FOOT_TOLERANCE;

    // Ghi láº¡i gÃ³c gá»‘i cho symmetry tracking
    if (ctx.state == Cmt3State.p4_ashwa_sanchalanasana) {
    ...
    if (_debouncer.update(isShearing)) {
      ...
      if (ctx.kneeAngle >= Cmt3Config.LUNGE_KNEE_ANGLE_RANGE[0] &&
          ctx.kneeAngle <= Cmt3Config.LUNGE_KNEE_ANGLE_RANGE[1]) {

...
class DowndogSpineMetric extends Cmt3MetricBase {
  @override
  ...
  void update(Cmt3Context ctx) {
    final isActive = ctx.state == Cmt3State.p5_parvatasana ||
        ctx.state == Cmt3State.p8_parvatasana_return;
    if (!isActive) return;

    final armAngle = ctx.armShoulderAngle;
    final isSpineCollapsed =
        armAngle < Cmt3Config.V_SHAPE_ARM_ANGLE_RANGE[0];
